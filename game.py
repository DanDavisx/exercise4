...
#  PROGRAM IS NOT VALIDATING TEXT (FIXED)
#  NONE APPEARING CANNOT FIND WHY (FIXED)
...
def remove_spaces(text):
    ...
    ''
    """
    no_space = text.strip()  # .strip() function removes outer spaces
    return no_space
...
def normalise_input(user_input):
    ...
    'help'
    """
    punct = remove_punct(user_input)  # remove punctuation first
    spaces = remove_spaces(punct)  # remove spaces
    lowerc = spaces.lower()  # convert to lower case
    return lowerc  # normalised successfully
...
def display_room(room):
    ...
    """
    print("")  # blank line
    print(room["name"].upper())  # read the room name + convert to uppercase
    print("")  # blank line
    print(room["description"])  # read the room description
    print("")  # blank line
...
def exit_leads_to(exits, direction):
    ...
    # ^ get the name of the room that direction leads to
...
def print_menu_line(direction, leads_to):
    ...
    # ^ simply prints "go DIRECTION to wherever the direction leads to."
...
def print_menu(exits):
    ...
    for direction, location in exits.items():
        # ^ loop through key (direction) and value (location) in the exits dict
        leads_to = exit_leads_to(exits, direction)
        # ^ leads_to calls exit_leads_to to get the room name
        print_menu_line(direction, leads_to)
        # ^ calling print menu line to print the direction and room name
...
def is_valid_exit(exits, user_input):
    ...
    if user_input in exits:
        ...
        return False  # returns true if the input is in exits, else false
...
def menu(exits):
    ...
    while True:  # loops until false
        print_menu(exits)  # call print_menu function
        user_input = normalise_input(input(str("")))  # ask for input and normalise
        if is_valid_exit(exits, user_input):  # check if input is valid
            return user_input  # if so return the user input
            #  ^ is_valid_input returns false so while loop ends
        else:
            print("That input isn't possible!")
...
def move(exits, direction):
    ...
    False
    """
    return rooms[exits[direction]]  # return the room dictionary that the direction leads to
